...
Voici la liste des commandes utilisables avec ce bot. Le préfixe par défaut est "+" mais il est modifiable dans config.py, tout comme les mots clés des commandes, dans main.py.  
    ...
    * *stat* : Envoie le nombre de message que l'auteur de la commande. Fonctionne aussi avec les mentions.
    * *edit* : Permet la modifications de plusieurs fichiers importants pour le bot : les csv, contenant des informations utiles pour hello.py ou encore timers.py. A complêter
    * *bissap* : envoie un gif de bissap
    * *sixseven*/*67* : envoie blehhh x3
    * *admin* : vérifie les permissions de l'utilisateur et permet d'executer d'autres commandes.
    * *admin test* : 
